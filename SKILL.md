...
- **Encumbered Key / Enclave Address** (`/v1/eth/address`, `/v1/eth/sign`) — An ephemeral key generated by Odyn inside the enclave at boot. Unique per enclave instance. The `enclave_address` in code refers to this key's Ethereum address. Use it to prove a response originated from inside this specific TEE instance. This is an **encumbered key** as defined in [ERC-733](https://draftv4.erc733.org) — a private key that exists only within the enclave and can never be extracted.
